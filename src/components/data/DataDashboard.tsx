/**
 * Data Dashboard - Avanza-inspired overview
 * 
 * Financial aesthetic meets societal diagnostics
 */
...
export const DataDashboard: React.FC<{ className?: string }> = ({ className }) => {
  ...
  const { data: kpiData } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data: values } = await supabase
        .from('kpi_values')
        .select('kpi_id, value, trend_percent, status')
  ...
  const recentUpdates = [
    {
      ...
      summary: 'Sysselsättningsgapet mellan inrikes och utrikes födda minskade med 1.2 procentenheter under fjärde kvartalet...',
  ...
  // Index ticker data
  const tickerData = [
    { code: 'λ-SE', name: 'Lambda Sverige', value: 0.94, change: -0.32 },
    { code: 'λ-EU', name: 'Lambda EU', value: 0.91, change: -0.18 },
    { code: 'λ-GLOB', name: 'Lambda Global', value: 0.87, change: -0.45 },
    { code: 'HDI-SE', name: 'HDI Sverige', value: 0.947, change: 0.12 },
    { code: 'GINI-SE', name: 'Gini Sverige', value: 28.8, change: 0.85 },
  ];
      ...
      <ScrollArea className="flex-1">
        <div className="p-6 max-w-7xl mx-auto">
          ...
          <div className="flex items-center justify-between mb-6">
            <div>
              <h1 className="text-2xl font-semibold">Just nu</h1>
          ...
          <div className="grid lg:grid-cols-3 gap-6">
            ...
            <div className="space-y-6">
                ...
                <CardContent className="p-4">
                  <div className="grid grid-cols-3 gap-2">
                    {[
                      ...
                      { label: 'Karta', path: '/gdm' },
                      { label: 'Diagnos', path: '/diagnostics' },
                    ].map((item) => (
                      <button
                        key={item.path}
                      ...
                      >
                        <span className="text-lg font-mono">[{item.label.charAt(0)}]</span>
              ...
              <Card className="bg-muted/30">
                <CardContent className="p-4">
                  ...
                  <p className="text-xs text-muted-foreground mb-3">
                    ...
                    inom hälso- och integrationssektorn. Se korrelationsanalys för detaljer.
                  </p>
                  <button 
                    onClick={() => navigate('/gdm')}
              ...
              <Card>
                <CardHeader className="pb-2">
                  <CardTitle className="text-base">Aktuellt fokus</CardTitle>
                </CardHeader>
                <CardContent className="space-y-3">
                  <div className="p-3 bg-amber-500/10 rounded-lg border border-amber-500/20">
                    <p className="text-sm font-medium text-amber-700 dark:text-amber-400">Februari 2026</p>
                    <p className="text-xs text-muted-foreground mt-1">
                      Q4 2025-data nu tillgänglig för 47 indikatorer. 
                      Majoriteten visar neutral till svagt negativ trend.
                    </p>
                  </div>
                  <button 
                    onClick={() => navigate('/diagnostics')}
